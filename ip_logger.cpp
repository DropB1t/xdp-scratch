...
static int handle_event(void*, void* data, size_t size) {
    if (size < sizeof(ip_pair)) return 0;
...
int main(int argc, char** argv) {
    if (argc != 3) {
        ...
        std::fprintf(stderr, "Example: %s eth0 ip_logger_filter.o\n", argv[0]);
        return 1;
    }
    ...
    bpf_map* rb_map = bpf_object__find_map_by_name(obj, "ring_buf");
    if (!rb_map) {
        std::fprintf(stderr, "Failed to find ringbuf map 'ring_buf'\n");
